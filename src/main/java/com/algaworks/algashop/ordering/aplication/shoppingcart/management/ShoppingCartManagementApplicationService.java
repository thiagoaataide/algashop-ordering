...
public class ShoppingCartManagementApplicationService {
    ...
    public void addItem(ShoppingCartItemInput input) {
        ...
        Product product = productCatalogService.ofId(new ProductId(input.getProductId())).orElseThrow(() -> new ProductNotFoundException());
    ...
    public void empty(UUID rawShoppingCartId){
        ShoppingCart shoppingCart = shoppingCarts.ofId(new ShoppingCartId(rawShoppingCartId)).orElseThrow(() -> new ShoppingCartNotFoundException());
        shoppingCart.empty();
        shoppingCarts.add(shoppingCart);
    }
